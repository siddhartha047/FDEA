...
public class MemParamEstimationSigMoidPareto {
	...
	public void Initialization(SolutionSet solutionSet){
		...
		for(int i=0;i<numberOfOjectives;i++){
			...
			A[i]=SigMoid.CalculateA(-ObjMean[i]+1.00*Objvar[i],.99,-ObjMean[i]);
			APos[i]=SigMoid.CalculateA(ObjMean[i]-1.00*Objvar[i],.99,ObjMean[i]);
	...
	public void FitnessAssignment(SolutionSet solutionSet) {
		...
		for (int i = 0; i < solutionSet.size(); i++) {
			...
			for (int j = i + 1; j < solutionSet.size(); j++) {
				...
				Solution comPareSolution = solutionSet.get(j);
				CompareSolutionPareto(solution, comPareSolution);
	...
	public void CompareSolutionPareto(Solution mainSolution, Solution compareSolution) {
		double[] mainObjectives = mainSolution.get_objectives();
		double[] compareObjectives = compareSolution.get_objectives();
		...
		if(better+equal==mainObjectives.length)dominatedBy=0;
		else if(worse+equal==mainObjectives.length)dominateBy=0;
