...
public class SidNaheed extends Algorithm {
	...
	public SidNaheed(Problem problem,double inwardGap) {
		super (problem) ;
		interval=inwardGap;
	}
...
/*
    ...
    public void PartitionByCrowdingDistance(SolutionSet population,SolutionSet union,int populationSize,Distance distance,boolean maintainDivergence){
    	...
    	int TakeExtra=(int)(((double)LeftOut)/interval);    	
    ...
    public void penalizeAndTakeSolution(SolutionSet population, SolutionSet front, int remain){
		...
		*/
		...
		while (evaluations < maxEvaluations) {
			...
			if (remain > 0) { // front contains individuals to insert
				...
				//Penalty.takeSolutionByPenalizeMethod(population,front,remain,better);
			...
			//		if(CombinedUsedTimes%interval==(interval-1)){maintainDivergence=true;}
			   ...
			   //better.FitnessAssignment(front);
				...
				//PartitionByCrowdingDistance(population,front,remain,distance,false);
				...
				*/
				...
				for(int i=0;i<remain;i++){
					population.add(front.get(front.size()-i-1));
				}
				...
				*/
				...
				//penalizeAndTakeSolution(population,front,remain);
