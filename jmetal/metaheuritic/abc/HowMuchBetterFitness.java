...
public class HowMuchBetterFitness {
	...
	public void UpdatedCompareSolution(Solution mainSolution, Solution compareSolution) {
		double[] mainObjectives = mainSolution.get_objectives();
		double[] compareObjectives = compareSolution.get_objectives();
		...
		double compareFit=compareSolution.getFitness();
		compareFit+=dominateByWR;
		
		compareSolution.setFitness(compareFit);
		
	}
	
	public void updateThePopulation(SolutionSet taken,SolutionSet front){
		
		for(int i=0;i<front.size();i++){
			front.get(i).setFitness(0);
		}
		
		
		for(int i=0;i<taken.size();i++){
			
			Solution sol1=taken.get(i);
			
			for(int j=0;j<front.size();j++){
				
				Solution sol2=front.get(j);
				
				UpdatedCompareSolution(sol1, sol2);
		...
		for(int i=0;i<front.size();i++){
			if(bestValue>front.get(i).getFitness()){
				bestValue=front.get(i).getFitness();
				bestOne=front.get(i);
				index=i;
			}
		}
