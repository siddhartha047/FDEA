  ...
  */
  ...
  public void printObjectivesToFile(String path,boolean printFitness){
	    try {
	      ...
	      for (int i = 0; i < solutionsList_.size(); i++) {
	        //if (this.vector[i].getFitness()<1.0) {
	        bw.write(solutionsList_.get(i).toString()+" "+solutionsList_.get(i).getFitness());
  ...
  public double [][] writeObjectivesToMatrix() {
    ...
    return objectives;
  	...
  	public void printSolutionSet(){
		for(int i=0;i<size();i++){
				printSingleSolution(get(i));
		}
		System.out.println();
		
	}
	
	public void printSingleSolution(Solution solution){
	...
	public void printFuzzySolutionSet() {
		// TODO Auto-generated method stub
		for(int i=0;i<size();i++){
			//printSingleSolution(get(i));
			Solution solution=get(i);
		
			double []objective=solution.get_fuzzy_objectives();
