...
import org.moeaframework.algorithm.sid.Sid;
...
public class StandardAlgorithms extends AlgorithmProvider {
	...
	public Algorithm getAlgorithm(String name, Properties properties,
		...
		try {
			if (name.equalsIgnoreCase("MOEAD")) {
				...
				return newSid(typedProperties, problem);
			}
			else {
				return null;
			}
	...
	private Algorithm newSid(TypedProperties properties, Problem problem) {
		int populationSize = (int)properties.getDouble("populationSize", 100);
		...
		HowMuchBetterFitness betterFitness=new HowMuchBetterFitness();
		CombineFitnessAssignment combine=new CombineFitnessAssignment(populationSize);
		
		SidNonDominatedSorting population = new SidNonDominatedSorting(betterFitness,combine);
		...
		//Variation variation = OperatorFactory.getInstance().getVariation("SBX", properties, problem);
		
		System.out.println(variation.toString());

		return new Sid(problem, population, null, selection, variation,
				initialization);
	}
