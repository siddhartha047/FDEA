...
import org.moeaframework.algorithm.sid.CombineFitnessAssignment;
import org.moeaframework.algorithm.sid.HowMuchBetterFitness;
import org.moeaframework.algorithm.sid.InvertFitnessComparator;
 ...
 */
public class SidNonDominatedSorting extends Population {

	private HowMuchBetterFitness betterFitness;
	private CombineFitnessAssignment combine;
	 ...
	 */
	public SidNonDominatedSorting() {
		this(new ParetoDominanceComparator());
	}
	
	public SidNonDominatedSorting(HowMuchBetterFitness better,CombineFitnessAssignment combineFitness)
	{
		this(new ParetoDominanceComparator());
		betterFitness=better;
		combine=combineFitness;
	}
	 ...
	 */
	public SidNonDominatedSorting(DominanceComparator comparator) {
		super();
	 ...
	 * @param iterable the solutions used to initialize this population
	 */
	public SidNonDominatedSorting(Iterable<? extends Solution> iterable) {
	...
	public void prune(int size) {
		...
		while (!remaining.isEmpty()) {
			...
			NondominatedPopulation front = new NondominatedPopulation(fastNondominatedSorting.getComparator());
			...
			if((remain > 0) && (remain >= front.size())) {
				...
				betterFitness.Initialization(front);
				betterFitness.FitnessAssignment(front);
				combine.Combine(front);
				
				takePopulation(front, remain);
				remain = 0;
	...
	public void takePopulation(NondominatedPopulation union,int populationSize){
        union.sort(new InvertFitnessComparator());  
        for(int i=0;i<populationSize;i++){
            this.add(union.get(i));
        }
	}	
